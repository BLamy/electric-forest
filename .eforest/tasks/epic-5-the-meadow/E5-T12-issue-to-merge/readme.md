...
priority: 512
status: pending
depends_on: [E5-T07, E5-T10, E5-T11]
estimate: L
capstone: true
---
...
fresh browser profiles — as one command: `make verify-E5-capstone`. The scenario
(driven by a committed demo script; this task *composes and proves* E5-T01…T11, it
implements no new meadow behavior): browser **A** (the actor) files an issue through
the E5-T04 dispatch hook, flips it to `in-progress` on the board, forks a fix branch
from `main` at a recorded offset (E1-T08 fork through the platform), lands a small fix
on the branch (stream-fs patch events), opens a PR whose `pr/opened` payload carries a
`closes` reference to the issue (E5-T06 frozen entity-ref), leaves a review comment,
approves, and merges through the E5-T08 PR page — the accepted `pr/merged` event drives
the E5-T05 log-aware merge onto `main` **and** its E5-T06 close propagation appends
`issue/state-changed { to: "done", via: { prStream, mergeOffset } }` to the issue,
exactly once. Browser **B** (the witness, a separate profile, separate authenticated
identity) holds the issue board, the issue detail, and the PR page open across the
whole run and observes **every** step live with zero reloads and zero document
navigations per watched surface — issue appears, flips to `in-progress`, PR appears
with its backlink, review comment and approval land, merge lands, issue flips to `done`
— each observation checkable via the E3-T02 DOM-exposed offset/digest contract. In the
same session, A attaches a piece of evidence (an event-log dump as an E5-T09 content
stream) to the issue and B sees it resolve live with its hash rendered and matching
(E5-T10); A edits a wiki page on the wiki branch (E5-T07) and B's open wiki view
updates live. **Verdict:** the full negotiation — issue stream, PR stream, fix-branch
stream, and `main` — is dumped and replayed offset-by-offset through the E5-T11
negotiation harness to one composite digest; that composite digest byte-equals the
value recomputed from the DOM-exposed per-entity digests captured in browser B at final
quiescence; the issue's `done` flip sits at exactly the offset whose event cites the
merge (`via.mergeOffset` string-equal to the `pr/merged` offset in the PR dump, E0-T03
opacity: string comparison only). And the founding bet is audited mechanically:
**Postgres count: zero** — a repo-wide dependency and source scan proves no database
anywhere in the tree (bet 4). Every artifact — per-entity golden logs, the composite
digest, the DOM captures, the wiki and evidence transcripts, the no-database audit —
is committed under `evidence/`, and the golden logs are promoted into the E5-T11
...
(E5-T03), the browser write path and live issue UI (E5-T04), merge execution through
the PR door (E5-T05), close propagation exactly-once (E5-T06), the wiki branch and
its live browser sync (E5-T07), the PR pages with review timeline and backlinks
(E5-T08), evidence attachments as content streams (E5-T09) rendered live with matching
hashes (E5-T10), and the multi-stream negotiation replay harness with its composite
digest and `verify-E5-negotiation` target (E5-T11). `depends_on: [E5-T07, E5-T10,
E5-T11]` is the transitive frontier: E5-T11 pulls in the entity/merge/linking chain
(T01–T06, T08), E5-T10 pulls in T09, E5-T07 stands alone as the wiki leg. If the demo
needs any frozen contract changed — an event envelope, the entity-ref shape, the
composite-digest recipe, the DOM offset/digest exposure — that is a refutation of the
earlier task, not a patch here.

Per `.eforest/tasks/README.md`, a capstone requires the demo performed end-to-end from
a cold start — no state left over from development. The run that produces the
...
conflict surfacing is E5-T05/T08's proven ground); no third browser; no CLI watcher
...
`.eforest/tasks/epic-5-the-meadow/E5-T12-issue-to-merge/`.
...
- `tools/verify/capstone_e5.sh` — the composed verdict: run the demo script against a
  ...
  branch and the evidence content stream; replay the negotiation through the E5-T11
...
- `tools/verify/no_database_audit.sh` — the bet-4 audit: scans every `package.json`
  in the workspace (dependency graph, including transitive lockfile entries) and all
  source for database clients/engines (`pg`, `postgres`, `mysql`, `sqlite`,
  `better-sqlite3`, `knex`, `prisma`, `typeorm`, `sequelize`, `mongodb`, `redis`,
  `leveldb`, connection-string patterns), prints `NO-DATABASE deps=0 sources=0 OK` or
  fails listing every hit; includes a self-test that plants a `pg` dependency in a
  scratch copy and asserts the audit goes red (`NO-DATABASE EXPECTED-FAIL OK`).
- `Makefile`: `verify-E5-capstone` inside the marker section — cold-clone semantics
  (scrubbed env, fresh server data dir, ephemeral port, fresh browser profiles), runs
  the demo script, the verdict script, the no-database audit, the sensitivity legs
  below, and re-runs `verify-E5-negotiation` (E5-T11) unmodified in the same clone;
  joins `verify-all`; `make verify-list` maps it to this task;
  `tools/verify/self_check.sh` still passes. The capstone must not fork the E5-T11
  harness — it feeds it.
- **Promoted golden logs**: the run's per-entity dumps committed into the E5-T11
  corpus (`packages/*/fixtures/negotiations/issue-to-merge/` per E5-T11's layout)
...
- Browser evidence: `tools/replay/record-run.sh -o e5-t12-final` — **one** Replay
...
- `evidence/` from the recorded cold-start run:
  - `e5-t12-transcript.txt` — the full `make verify-E5-capstone` transcript, including
    `LIVENESS_BOUND_S`, every greppable verdict line, and the `verify-E5-negotiation`
    re-run.
  - `e5-t12-issue-log.jsonl`, `e5-t12-pr-log.jsonl`, `e5-t12-branch-log.jsonl`,
    `e5-t12-main-log.jsonl`, `e5-t12-wiki-log.jsonl`, `e5-t12-evidence-stream.jsonl`
    — the complete dumps, each with a `.sha256` sibling.
  - `e5-t12-digests.txt` — the composite digest from the E5-T11 replay, the composite
  ...
  - `e5-t12-timeline.txt` — the machine-readable step timeline: each actor dispatch's
    offset, B's witnessed offset and wall-clock latency per surface, wiki and
    attachment observation marks.
  - `e5-t12-no-database.txt` — the full audit output including the planted-dependency
    self-test.
  - `e5-t12-sensitivity.md` — the sabotage transcript (below).

## Acceptance criteria

- [ ] Cold start: `make verify-E5-capstone` exits 0 from a pristine clone via
      ...
      `make verify-E5-capstone 2>&1 | grep -c '^SKIPPED:'` prints `0`; transcript
      committed as `evidence/e5-t12-transcript.txt`.
- [ ] **Every step witnessed live.** For each of the nine actor steps (issue filed,
      ...
      evidence: `make verify-E5-capstone 2>&1 | grep -c '^STEP .* OK$'` prints ≥ `10`,
      per-step latencies in `evidence/e5-t12-timeline.txt`, the bound a committed
      constant that predates the run (git history), and the navigation/console
      assertions in the committed spec.
- [ ] **The merge closes the issue, exactly once, at the cited offset.** The issue
      ...
      the issue dump (the flip came from propagation, not a hand edit) — evidence:
      `make verify-E5-capstone 2>&1 | grep -c '^CLOSE offset=.* via=.* count=1 OK$'`
      prints `1`, offsets in `evidence/e5-t12-digests.txt`.
- [ ] **Composite digest, two instruments.** The E5-T11 harness replays the committed
      multi-stream dumps to a composite digest, and the composite recomputed from
      browser B's DOM-exposed per-entity digests captured at final quiescence
      byte-equals it — evidence:
      `make verify-E5-capstone 2>&1 | grep -c '^COMPOSITE digest=.* dom=.* OK$'`
      prints `1` with the two fields equal; both values in
      `evidence/e5-t12-digests.txt`.
- [ ] **Merge integrity.** `ef replay` of the committed main dump yields a tree digest
      ...
      the verdict script, values in `evidence/e5-t12-digests.txt`.
- [ ] **Evidence attachment resolves.** The attachment rendered in browser B shows a
      ...
      `make verify-E5-capstone 2>&1 | grep -c '^ATTACH sha256=.* dom=.* OK$'` prints
      `1` with the fields equal.
- [ ] **Wiki live-sync in the same session.** A's wiki edit appears in B's open wiki
      ...
      `make verify-E5-capstone 2>&1 | grep -c '^WIKI offset=.* witnessed OK$'`
      prints `1`.
- [ ] **Golden logs promoted.** The run's dumps are committed into the E5-T11 corpus
      ...
      `evidence/e5-t12-transcript.txt`.
- [ ] **Postgres count: zero.** `tools/verify/no_database_audit.sh` scans every
      workspace `package.json`, the lockfile's transitive graph, and all source, and
      prints `NO-DATABASE deps=0 sources=0 OK`; its planted-dependency self-test goes
      red first — evidence:
      `make verify-E5-capstone 2>&1 | grep -c '^NO-DATABASE deps=0 sources=0 OK$'`
      prints `1` and `grep -c '^NO-DATABASE EXPECTED-FAIL OK$'` prints `1`; full
      output in `evidence/e5-t12-no-database.txt`.
- [ ] **One recording, both sessions.** The cited Replay recording (fresh profiles,
      `-o e5-t12-final`) contains both browser contexts across the entire flow:
      ...
      and B's final DOM digests equal `evidence/e5-t12-digests.txt`. URL cited in the
      Verification log.
- [ ] **Sensitivity.** `verify-E5-capstone`'s sabotage step runs in a scratch worktree
      under each of: (a) E5-T06 close propagation disabled — must redden the `CLOSE`
      count assertion (issue never flips); (b) one byte flipped in a copy of the
      committed issue dump — the E5-T11 replay leg must go red at that offset; (c)
      browser B's live tail replaced by a reload-on-poll — must redden the
      ...
      `make verify-E5-capstone 2>&1 | grep -c 'EXPECTED-FAIL OK'` ≥ `5` (four legs
      plus the no-database self-test), transcript in `evidence/e5-t12-sensitivity.md`.
- [ ] Nothing forked: the full E5-T01…T11 verify targets re-run unmodified and green
      ...
      `make verify-list` maps `verify-E5-capstone` to this task; `verify-all` green —
      evidence: each target's name and exit status appended to
      `evidence/e5-t12-transcript.txt`.
...
1. **Cold start or it didn't happen (mandatory).** Run `make verify-E5-capstone` only
   via `tools/verify/cold_clone.sh` on a machine/profile the builder didn't prepare:
   scrub `EF_*`/`REPLAY_*` env beyond the script's scrub, point `HOME` at a temp dir,
   verify the server data dir and both browser profiles are created during the run
   (`find` before/after). Any pre-created org/repo/issue, cached token, or warm
   profile refutes the cold-start claim outright.
...
3. **Provenance of the flip.** Dump the issue stream yourself from the run's data dir
   ...
   propagation alone). A script that dispatches the close itself — even redundantly —
   refutes the headline moment.
...
5. **Your own negotiation, same skeleton.** Re-run the demo with your own content:
   ...
   Then replay your run's dumps through the E5-T11 harness twice from scratch —
   composite digests must be byte-identical; divergence refutes determinism and is
   filed against E5-T11 with your dumps as the repro.
6. **Database hunt beyond the audit.** Run the no-database audit, then go past it:
   ...
   a `node:sqlite` import refutes the bet-4 apparatus.
7. **Kill it mid-negotiation.** SIGKILL the server between the PR approval and the
   ...
   exactly once (E5-T05/T06 claim crash-safety through the door; the capstone
...
8. **Replay interrogation.** Open the cited recording via the Replay MCP: confirm
   ...
   `e5-t12-digests.txt`, refutes the browser claim.
9. **Evidence provenance.** Re-derive everything committed: replay each committed
   dump to its `.sha256`, run the E5-T11 harness on the promoted goldens and match
...
transitive graph, an offset in the evidence that resolves to nothing, or a cold clone
that needed warm state. "The demo felt smooth" is a caption, not a finding.
