...
pins the divergence at exactly the merge event's offset. `make verify-E5-T05` proves all
of it cold.
...
merge event (ROADMAP Epic 5), and the E5-T12 capstone's "review + approve, merge — the
issue flips to done via the merge" walks straight through this executor. E5-T06
(closes-references flipping issues) and E5-T08 (the PR web surface with merge button and
...
`<!-- frozen:E5-T05:<block> -->` markers, doc-sync checked like E1-T10's):

<!-- frozen:E5-T05:outcome-events -->
...
<!-- frozen:E5-T05:gate-and-refusals -->
...
Non-goals: no web surface (E5-T08), no issue-closing side effects (E5-T06), no conflict
...
- [ ] **Three outcomes, golden.** Each fixture replays to its expected outcome —
      ...
      digest, and the conflict line's `state=conflicted` attests the PR reducer's final
      state with the target's conflicted paths still holding pre-merge bytes.
- [ ] **Conflicts are events on both streams.** The `conflict` fixture's post-merge
      ...
      event's actual offset — evidence: committed test assertions comparing the two
      dumps' canonical events, `pnpm test --filter @eforest/meadow` exit 0.
- [ ] **Refusals append nothing anywhere.** Every `refusals` fixture case — unapproved,
      ...
      `approved` — evidence:
      `make verify-E5-T05 2>&1 | grep -c '^REFUSAL case=.* pr-head=unchanged target-head=unchanged OK$'`
...
- [ ] **Bisect pins the merge.** For the `ff` and `three-way-clean` fixtures,
      ...
      `make verify-E5-T05 2>&1 | grep -c '^BISECT fixture=.* OK$'` prints `2`, each
...
- [ ] **Sensitivity.** The mutation leg runs inside `make verify-E5-T05` — evidence:
      `make verify-E5-T05 2>&1 | grep -c '^MUTATION fixture=.*byte=.*EXPECTED-FAIL OK$'`
...
- [ ] All workspace gates pass repo-wide: `pnpm format:check && pnpm lint &&
      ...
      `verify-E5-T05`; `tools/verify/self_check.sh` passes.
- [ ] Durable evidence committed under this task's `evidence/`: the final
      `make verify-E5-T05` transcript (all prmerge, REFUSAL, BISECT, MUTATION lines),
...
- [ ] Replay browser layer: N/A (no browser-reaching surface; the PR web UI is
      E5-T08) — mitigation: stream-layer evidence above is the currency; the
...
2. **Conflict-suppression sabotage.** In a disposable worktree: (a) make the executor
   ...
   `make verify-E5-T05` must both go red. A sabotage that stays green refutes the
...
6. **Golden rot and cold clone.** Regenerate the fixtures' expected digests from the
   ...
   `make verify-E5-T05` via `tools/verify/cold_clone.sh` twice back-to-back with
   ...
   merge semantics, no auto-resolution, no UI.
