...
the entities it proves things about. E5-T10 renders exactly this model in the UI
(hashes shown and matching, links resolving); E5-T11's negotiation harness attaches its
...
invalidates every evidence golden and E5-T10/E5-T11's citations):
...
- **Reduced shapes**, canonical-JSON encoded and digested per E0-T03, no wall-clock or
  ...
  hiding them is E5-T10's rendering choice); `ContentState { v: 1, size, chunks,
...
Non-goals: rendering attachments in the web app and browser-side upload UX (E5-T10),
the negotiation-harness composite digest (E5-T11), attaching to Epic-6 task entities
...
`.eforest/tasks/epic-5-the-meadow/E5-T09-evidence-attachment-model/`.
...
- `packages/evidence` (`@eforest/evidence`):
  ...
  - `src/upload.ts` — `uploadAttachment(client, { entityRef, kind, name, mediaType,
    ...
    returns the exact original bytes (the round-trip mouth E5-T10 and tests reuse).
...
- Committed evidence:
  - `evidence/e5-t09-attachments.jsonl` + `evidence/e5-t09-content.jsonl` — golden
  ...
  - `evidence/e5-t09-digests.txt` — their committed `ef replay --digest` values,
    produced by two separate processes, plus the sealed artifact's SHA-256.
  - `evidence/e5-t09-roundtrip.txt` — transcript: upload → download → `cmp` exit 0
    against the original file → `shasum -a 256` equal to the sealed digest → the
    replayed `ContentState.sha256` equal to both.
  - `evidence/e5-t09-refusals.txt` — one block per reason code: dispatch body,
    response, before/after head offset + dump digest, byte-equal.
  - `evidence/e5-t09-sensitivity.md` — sabotage transcripts.
- `Makefile`: `verify-E5-T09` per the E0-T02 target contract — cold-clone runnable,
  joins `verify-all`, `make verify-list` maps it: (1) replays both goldens twice as
  separate processes and diffs against `e5-t09-digests.txt` (never regenerates); (2)
  ...
  `MUTATION fixture=e5-t09-content byte=<offset> digest-mismatch EXPECTED-FAIL OK`
...
- [ ] `make verify-E5-T09` exits 0 from a pristine cold clone via
      ...
      `make verify-E5-T09 2>&1 | grep -c '^SKIPPED:'` prints `0`.
- [ ] **Replay determinism of the attachment list**:
      `ef replay evidence/e5-t09-attachments.jsonl --digest` (reducer resolved as
      `evidence`) run twice in two separate node processes prints one byte-identical
      digest equal to the committed value in `evidence/e5-t09-digests.txt`; same for
...
- [ ] **Byte-parity round trip**: an uploaded event-log artifact downloaded via
      ...
      `evidence/e5-t09-roundtrip.txt` plus the in-target re-run.
- [ ] **Integrity sensitivity**: the in-target mutation step flips one byte of a chunk
      ...
      go red — evidence: `make verify-E5-T09 2>&1 | grep -c
...
- [ ] **Every refusal is log-neutral**: all fourteen frozen reason codes produced
      ...
      the fresh-driven set diffs byte-equal against `evidence/e5-t09-refusals.txt`.
...
- [ ] **Reference events carry no bytes**: `evidence.linked` with a valid `https:`
      ...
      `contentStream`/`sha256`; `http:`, `javascript:`, `data:`, and 2049-char URLs
...
- [ ] Replay browser layer: N/A — server/package task with no browser-reaching
      surface (evidence rendering is E5-T10); declared explicitly per AGENTS.md, with
...
3. **Byte-flip sweep.** Take your own sealed content dump and flip one byte in every
   ...
   digest comparison in `verify-E5-T09` goes red, not regenerate-and-pass.
...
7. **URL smuggling.** `evidence.linked` with `javascript:alert(1)`,
   ...
   smuggled URL refutes — it becomes E5-T10's XSS, forever, because the log is
   forever.
...
10. **Sabotage the suite.** In a scratch worktree: (a) make the seal validator trust
    ...
    `make verify-E5-T09` must go red. Any sabotage that stays green refutes whichever
    gate it slipped past. Check the diff for `.skip`/`.todo`/lint disables while
    there. Then the self-licking check: `git log evidence/e5-t09-digests.txt` — the
