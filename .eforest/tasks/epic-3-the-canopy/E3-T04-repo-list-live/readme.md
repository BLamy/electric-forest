...
deterministic seed corpus with golden per-stream digests — the corpus as E3-T01 froze it
includes two orgs and mixed public/private repos owned by two distinct E2-T01 identities,
so this page has something worth asserting; E3-T01's spec contains **no** `ns.repo.rename`
(its only renames are fs-level renames inside `reading-room@main`), so the repo-rename
fixture this task needs is **owned by this task**: `tools/verify/e3-t04/second-client.ts`
dispatches the `ns.repo.rename` in-run during `make verify-E3-T04`, capturing the
creation-time `repoStreamPrefix` in the same run — E3-T01's frozen deliverable is not
amended; both E3-T01 and E3-T02 reached transitively through E3-T03's dependency
closure), **E2-T08** (the `__registry__` derived stream, the
...
- `tools/verify/e3-t04/second-client.ts` (or equivalent under the task's verify
  ...
  for appearance), the private creation (for the absence probe), and the
  `ns.repo.rename` for the row-link criterion (capturing the creation-time
  `repoStreamPrefix` in the same run) through the E0-T11 dispatch door with an E2-T05
  token, printing dispatch-accept timestamps and the resulting `__registry__` offsets
  for the transcripts.
- `evidence/goldens/` — **frozen per-identity expected-row goldens committed by this
  task**: `listing-owner.json`, `listing-member.json`, `listing-nonmember.json`,
  `listing-anonymous.json`, each recording the exact expected rows (entries, `(org,
  repo)` order) plus the expected registry state digest for that identity's view of the
  E3-T01 seed corpus, and each pinned to the E3-T01 corpus-manifest digest so corpus
  drift invalidates them. These are committed before the verify run and read-only
  during it — a golden regenerated by the code under test at test time proves nothing.
...
- [ ] Render parity on the seed corpus: for each corpus identity (at minimum: an owner,
      ...
      literal-equal **this task's committed frozen golden** for that identity —
      `evidence/goldens/listing-owner.json`, `listing-member.json`,
      `listing-nonmember.json`, `listing-anonymous.json` respectively (same entries,
      same `(org, repo)` order) — after the golden's pinned E3-T01 corpus-manifest
      digest is checked against the manifest of the corpus actually seeded in-run
      (mismatch fails the run); and the list region's `data-ef-digest` equals both
      (a) the digest recorded in that committed golden and (b) `ef replay --digest`
      over the `__registry__` dump taken at the region's `data-ef-offset`. The goldens
      are read-only during the verify run: the harness must be structurally unable to
      pass by regenerating a golden at test time (any write under `evidence/goldens/`
      during the run fails it). Transcript in `evidence/e3-t04-render-parity.txt`.
...
- [ ] No side store: the committed sweep greps **both the task's source diff and the
      built webapp bundle** for these exact patterns — `localStorage`,
      `sessionStorage`, `indexedDB`, `caches.`, `navigator.serviceWorker`,
      `BroadcastChannel`, `SharedWorker`, and imports of any storage library — exiting
      nonzero on any unwaived hit. A waiver is a committed line in
      `evidence/e3-t04-no-side-store.txt` naming the exact hit with a one-line reason,
      and the sweep asserts the waiver count equals the expected count (**0**, unless a
      Verification log entry argues for a specific higher number). Module-level mutable
      caches of registry entries are a semantic property the grep cannot see: they are
      checked **only** by the behavioral probes — after building list state, a hard
      reload with the server stopped renders no rows (loading/error state, not stale
      entries), a reload with the server up rehydrates to the same digest, and
      sensitivity proof (d) shows a reintroduced cache turns a probe red. Transcript in
      `evidence/e3-t04-no-side-store.txt`.
- [ ] Row links resolve and survive rename: every rendered row's href is the E3-T05
      route string for its `(org, repo)`. The rename fixture is this task's own, in-run:
      `tools/verify/e3-t04/second-client.ts` creates a repo (its `repoStreamPrefix`
      captured at creation time in the same run) and then dispatches `ns.repo.rename`
      while the list is open; after the rename the row shows the new name, links via
      the new name, and the entry's `repoStreamPrefix` (asserted from hook state
      in-page) is byte-identical to the creation-time value captured in-run. Evidence:
      the per-row href table (rendered name, href, expected E3-T05 route string) and
      the `repoStreamPrefix` byte-comparison (creation-time value, current value, `cmp`
      result) are appended to `evidence/e3-t04-render-parity.txt`, and the rename
      moment is anchored (point/time) in the `e3-t04-final` Replay recording.
...
6. **Rename adversary.** With the list open live, rename a repo a→b→a from your node
   ...
   alongside new) may exist at any settle point. Staleness is a **pure string check**
   (E3-T05 is pending, so every href 404s at verification time — navigation proves
   nothing): after each rename hop, an href whose `(org, repo)` path segments differ
   from the entry's current name in hook state refutes, as does a prefix that followed
   the rename; navigation-level resolution of the route is deferred to E3-T05's own
   verification.
