...
### 2026-07-23 — builder — rework claim (run 6)

Commit: 8fb402c (the run-6 rework commit; the claim head is the
apparatus+claim commit that follows it — run-5 ordering — and the
to-completion cold-clone transcript recorded at that claim head lands in
the commit after it, so sweep, cold clone, and claim stay mutually
consistent at every head). Surgical run: the run-5 judge confirmed every
run-4 behavior survived, so this diff touches ONLY the two refuted
sensor-coverage hunks plus the non-blocking ENV demand — no behavior a
surviving criterion depends on changed.

- SABOTAGE (stdin 'error' listener sensor-free) — sensor promoted. The
  critics' probe shape (work/critic-run5/probe-pending-write-crash.mjs)
  is now a committed unit test: ns.test.ts "survives a
  pipe-buffer-exceeding write left in flight at terminate()" warms the
  boundary, leaves an 8 MB isName request in flight (cannot flush into
  the pipe in one tick), terminates in the SAME tick, asserts the
  in-flight rejection ("namespace runtime terminated"), holds the test
  open 400 ms across the stream-error window, and asserts the owner
  still enforces the post-termination contract. Sensitivity proven under
  the verdict's exact sabotage: with ONLY namespace-runtime.ts:75 (the
  stdin 'error' listener) deleted and dist rebuilt, vitest exits 1 with
  unhandled 'write EPIPE' pinned to this test
  (work/builder-run6/sab-nolistener-vitest.log); restored, 21/21 green
  (work/builder-run6/head-vitest.log). The critics' own probe re-run at
  head: 3/3 exit 0, pending write rejected, owner alive.
- COVERAGE (spawn re-kill arm) — deleted, clause retracted (the
  verdict's preferred close). terminate() now issues one kill('SIGKILL')
  with a comment recording WHY no recovery arm exists: on this runtime
  spawn() assigns the pid synchronously, so a same-tick kill() is always
  deliverable while the child lives, failed spawn never emits 'spawn',
  and a post-exit kill() is a harmless no-op. The run-5 claim's "Worker
  reaping hardened" clause is retracted in place with a bracketed marker
  pointing here (the judge's verdict quotes the original clause, so the
  refuted text stays on the record). All 21 ns tests and the full suite
  stay green with the arm gone — consistent with the verdict's finding
  that it fired zero times.
- ENV (orphan load-driver spinners, non-blocking) — hardened + hazard
  noted. Both load drivers (builder's work/run5-clean-exit-driver.mjs
  AND the critics' work/critic-run5/critic-hang-driver.mjs) now spawn
  parent-pid-watchdog spinners: each spinner polls process.ppid inside
  its spin loop and self-exits once reparented to init, so a SIGKILLed
  driver — whose finally-block reaping never runs — cannot leak orphan
  load (probe: work/builder-run6/orphan-watchdog-probe.mjs — spinner
  self-exited ~500 ms after its driver was SIGKILLed). HAZARD, for the
  record: plain `for(;;)` spinner children reparent to PPID 1 when their
  driver dies by SIGKILL; 26 such orphans pushed ambient 1-min load to
  70.76 and timed out one pristine-clone test run in run 5. These
  drivers are gitignored task scratch; this entry is the durable record
  of the hardened shape.

Standing-apparatus updates forced by this diff (run-1..5 precedent): the
E2-T06 runtime-boundary manifest re-pins namespace-runtime.ts's content
digest (dca7618f… → 070fada1…; both hunks live in that pinned boundary
file); the E2-T06 sensitivity control line re-records ns.test.ts at 21
tests (20 → 21, the promoted pending-write test) via that harness's own
`--update-evidence` door, run against the committed rework tree; the
E2-T08 no-database sweep regenerated byte-identical (no new files, no
new tells — violations=0); the E1-T11 provenance closure re-verified
FLAGLESS (exit 0, work/builder-run6/run6-provenance-flagless.log — this
diff touches no cli/client program artifact, so no re-pin door was
needed); the E2-T06 no-database evidence regenerated byte-identical
under the re-pinned manifest (unallowlisted=0 stale=0).

Commands (all green, in order, at the claim head): `pnpm format:check &&
pnpm lint` → `pnpm typecheck` → `pnpm test` (391 tests; 390 → 391: +1
promoted pending-write sensor) → `pnpm build` → `CI=true make
verify-E2-T08` (work/builder-run6/run6-make-verify.log,
`verify-E2-T08: OK`; includes the full verify-E2-T07/E2-T06 re-runs and
the eight-sabotage sensitivity proof) →
`tools/verify/cold_clone.sh verify-E2-T08` to completion at the claim
head, zero `SKIPPED:` lines (transcript: `evidence/e2-t08-cold-clone.txt`,
committed in the immediately following commit).

Evidence: no new committed evidence files this run (the promoted sensor
lives in ns.test.ts; sabotage/probe logs are task scratch under
work/builder-run6/). Byte-compared and unchanged by this diff:
e2-t08-destruction.txt, e2-t08-refusal-neutrality.txt,
e2-t08-crash-idempotence.txt, e2-t08-visibility-matrix.txt,
e2-t08-sensitivity.md, e2-t08-no-database.txt,
e2-t08-refusals-clean-exit.txt. Structurally validated (pids/timestamps
vary by construction; digests and offsets asserted): e2-t08-live-tail.txt,
e2-t08-rebuild-determinism.txt.

Replay: N/A (no browser-reaching surface until E3 — server internals and
stream-layer doors only) + mitigation: the promoted red-under-sabotage
