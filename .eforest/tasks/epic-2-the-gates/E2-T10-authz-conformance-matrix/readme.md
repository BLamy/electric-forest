...
`cell-id | identity | operation | sub-probe | visibility | status | error.class | error.reason | listing`
(allow cells record the frozen success status and `-` for class/reason; no timestamps,
ports, offsets, or token material may appear in the line). For `registry-list` — and
any other operation not addressed to a specific repo — the visibility axis is defined
as: does the fixture repo of that visibility (`public` → the public fixture repo,
`private` → the private fixture repo) appear in the set the door returns to this
identity? The `listing` column is the policy-bearing answer: `present` or `absent`
for list-shaped cells, derived independently by the deriver from E2-T01's reduced
view joined with E2-T08's visibility policy and asserted against the actual returned
set, so a private repo leaking into any identity's listing is a byte diff in the
golden, not an invisible `200`. Repo-addressed operations record `-` in `listing`.
The transcript diffs
...
- **Decision-line grammar.** The canonical line format above (grammar v1: nine
  columns, the ninth being `listing`), one line per sub-probe,
  ...
  reason, and `listing` are never scrubbed. The `listing` column carries
  `present`/`absent` for list-shaped cells (per the Goal's registry-list semantics)
  and `-` for repo-addressed cells, so list-visibility policy is decision-bearing in
  every line, allow or deny. The grammar is documented beside the golden and any
  change to it — including any change to the `listing` semantics — bumps the grammar
  version in the fixture README and invalidates the golden.
...
- **Standing-gate registration.** `verify-E2-authz` appears in the Makefile verify
  ...
  task-numbered alias), is included in `verify-all`'s prerequisite list, and
  surfaces in the generated standing-verification inventory (`make verify-list`,
  which golden-sweep consumes). Deregistering it later requires a task
  that says so.
...
- `Makefile`: `verify-E2-authz` (fixture replay, derive, sweep, byte-diff, neutrality
  assertions, sensitivity script) and `verify-E2-T10` as its alias; joined to
  `verify-all`'s prerequisite list and added to the `.PHONY` verify declarations so
  the generated inventory (`make verify-list`, via `tools/verify/list.sh` scanning
  the Makefile verify targets) maps `verify-E2-T10` → E2-T10 with exit 0 — the
  E0-T02 target contract's registration mechanism.
- `evidence/` — the full 98-line (or larger) matrix transcript
  ...
  cell sets (`e2-t10-sensitivity.md`), the determinism proof
  (`e2-t10-determinism.txt` — transcript digests of the two consecutive runs, the
  shuffle-flag invocations with their printed seeds, and the byte-diff results),
  and the cold-clone run log (`e2-t10-cold-clone.txt`).
...
- [ ] Standing-gate registration, three mechanical checks: (1) `verify-E2-authz`
      appears in `verify-all`'s prerequisite list in the Makefile verify section
      (grep the Makefile, then run `make verify-all` and see the target execute);
      (2) `make verify-list` (the generated inventory, `tools/verify/list.sh`
      scanning the Makefile verify targets) exits 0 and its output maps
      `verify-E2-T10` → E2-T10; (3) a golden-sweep invocation's transcript shows
      `verify-E2-authz` executed via that entry point. `verify-E2-T10` aliases
      `verify-E2-authz` in the Makefile.
- [ ] Determinism: two consecutive `verify-E2-authz` runs from the same clone produce
      ...
      cells are independent, not order-coupled. Stream-layer evidence committed to
      `evidence/e2-t10-determinism.txt`: the digests of both consecutive runs'
      transcripts, the shuffle-flag invocations with their printed seeds, and the
      byte-diff results (all `diff` exit 0).
- [ ] Zero policy changes: `git diff` for this task adds or modifies files only
      ...
      `tools/verify/`, this task's own folder
      `.eforest/tasks/epic-2-the-gates/E2-T10-authz-conformance-matrix/` (readme
      frontmatter, Verification log, and its `evidence/` subdirectory — the
      `evidence/` paths named throughout this spec resolve there), the regenerated
      `.eforest/tasks/QUEUE.md` (bookkeeping via `python3 tools/build_queue.py`),
      and the Makefile verify recipes; any hunk elsewhere fails — in particular
      there is no diff under the server decision
