...
for (const entry of requests) {
  ...
  if (response.status !== entry.expect.status)
    fail(entry.name, `status ${response.status} != ${entry.expect.status}`);
  for (const [key, expected] of Object.entries(entry.expect.headers ?? {})) {
    const actual = response.headers.get(key);
    if (actual !== expected)
      fail(entry.name, `header ${key} ${JSON.stringify(actual)} != ${JSON.stringify(expected)}`);
  }
  if (actualBody !== (entry.expect.body ?? "")) {
    fail(
      entry.name,
      `body ${JSON.stringify(actualBody)} != ${JSON.stringify(entry.expect.body ?? "")}`,
    );
  }
...
if (replayResult.status !== 0 || replayResult.stdout.trim() !== clientDigest) {
  ...
  fail(
    "dump",
    `server replay digest ${JSON.stringify(replayResult.stdout)} != client digest ${clientDigest}`,
  );
}
rmSync(temp, { recursive: true, force: true });
