...
function run(name, args) {
  ...
  const extra = name === "mixed" ? [
    "--loser-output", join(scratch, "loser.bin"),
    "--conflict-output", join(scratch, "conflict.bin"),
  ] : [];
  const stdout = execFileSync(process.execPath, [
    join(root, "tools/verify/e4-sync/run.mjs"),
    "--seed", "1", "--profile", "offline", "--mode", "free",
    ...(name === "mixed" ? ["--scenario", "mixed"] : []),
    "--out", output, "--branch-dump", branch, ...extra,
...
for (const [label, final] of [["live", liveFinal], ["mixed", mixedFinal]]) {
...
writeFileSync(join(evidence, "e4-t12-transcript.txt"), [
...
writeFileSync(join(evidence, "e4-t12-branch-log.sha256"), `${branchSha}  e4-t12-branch-log.jsonl\n`);
writeFileSync(join(evidence, "e4-t12-digests.txt"), [
...
writeFileSync(join(evidence, "e4-t12-diff-A-vs-replay.txt"), "diff -r A materialized-replay --exclude .ef\n(empty)\n");
writeFileSync(join(evidence, "e4-t12-conflict.txt"), [
  "scenario=mixed",
  "conflictEvents=1",
...
writeFileSync(join(evidence, "e4-t12-partition-timeline.txt"), [
...
writeFileSync(join(evidence, "e4-t12-sensitivity.md"), [
...
console.log(`E4-T12 capstone: live=${liveFinal.digestA} mixed=${mixedFinal.digestA} conflict-events=1 branch-sha256=${branchSha}`);
